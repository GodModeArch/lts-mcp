...
export function registerReadTools(server: McpServer, client: SupabaseClient, meta: ApiMeta) {
  ...
  server.tool(
    ...
    {
      query: z.string().min(2).describe("Search term: project name, LTS number, developer name, or city. Matched as a literal substring; use * as a wildcard. A term of only wildcards matches nothing"),
  ...
  server.tool(
    ...
    "Browse LTS records from DHSUD with filters. Shows normalized records with confidence levels. Filter by confidence (high/medium), linked status (has project_id), region, or text search. Use expiringWithinDays to find records expiring soon.",
    {
      ...
      search: z.string().optional().describe("Text search: project name, LTS number, or developer. Matched as a literal substring; use * as a wildcard. A term of only wildcards matches nothing"),
  ...
  server.tool(
    ...
    "Get the complete LTS picture for a single project: all LTS records with computed fields (is_expired, days_until_expiry), summary counts, and the primary LTS number. Pass either a project UUID or a project name (fuzzy matched).",
    {
      ...
      projectName: z.string().optional().describe("Project name or slug for fuzzy lookup. Matched as a literal substring; use * as a wildcard. A term of only wildcards matches nothing. Use when you don't have the UUID"),
    },
